...
def load():
    ...
    console = RScodrev(2)
    bios = telecharger_fichier("BIOS.py", "")
    if comparatif("BIOS.py", telecharger_fichier("BIOS.py")) == False:
        insert_code_into_file('''def insert_code_into_file(source_codea, target_file):
source_code = f"""{source_codea}"""
with open(target_file, 'w') as file:
file.write(source_code + '\n')''' + "\n" + f"""insert_code_to_file({bios}, "BIOS.py")""")
